	    ...
	    document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/ajax-loader.gif" />';	
			  ...
			  success: function(res)
			  {
			      ...
			      if(resval[0]=='fail'){
				   ...
				   document.location='<?php echo $this->webroot; ?>Reports/add_report_view/'+resval[1];
...
function check_character(){
	    ...
	    document.getElementById('summary').value=trim(summary_holder);
...
<input type="text" id="event_date" name="event_date"  onclick="displayDatePicker('event_date',this);" value="<?php echo $event_date; ?>" /><span class="textcmpul" id="event_date_error"></span>
          ...
          <span id="client_section">
			   <span id="client_section">
			    <select id="client" name="client">	
			     ...
			     <?php } ?>
			     <option  value="0" <?php if($client==0){echo "selected";}else{} ?>>N/A</option>
			     </select>
...
<label><?PHP echo __("Incident Severity:");?></label>
			    ...
			    <select id="incident_severity" name="incident_severity">
			     ...
			     <option <?php echo  $incidentDetail[$i]['Incident']['color_code']; ?> value="<?php echo $incidentSeverityDetail[$i]['IncidentSeverity']['id']; ?>" <?php if($incident_severity==$incidentSeverityDetail[$i]['IncidentSeverity']['id']){echo "selected";}else{} ?> ><?php echo $incidentSeverityDetail[$i]['IncidentSeverity']['type']; ?></option>
			     <?php } ?>
			     </select>
...
<label><?PHP echo __("Potential:");?></label>
			    ...
			    <select id="potential" name="potential">	
                                <option value="1" <?php if($potential==1){echo "selected";}else{} ?>>High</option>
				<option value="2"  <?php if($potential==2){echo "selected";}else{} ?> >Medium</option>
				<option value="3" <?php if($potential==3){echo "selected";}else{} ?>>Low</option>
			    </select>
...
<div class="clearflds"></div>			 
<label><?PHP echo __("Summary:");?></label> <?PHP echo $this->Form->input('summary', array('type'=>'textarea', 'id'=>'summary','value'=>$summary, 'size'=>30,'maxlength'=>'40', 'label' => false,'div' => false, "onkeyup" =>"check_character();")); ?>
  
<div class="clearflds"></div>			 
<label><?PHP echo __("Details:");?></label>  <?PHP echo $this->Form->input('details', array('type'=>'textarea', 'id'=>'details','value'=>$details, 'size'=>30,'maxlength'=>'40', 'label' => false,'div' => false)); ?>
<div class="clearflds"></div>		  
